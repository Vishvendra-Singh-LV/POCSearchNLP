...
namespace POCSearchNLP.Controllers
{
	public class HomeController : Controller
	{
        ...
        private async Task<List<Dictionary<string, object>>> ExecuteSqlQueryAsync(string sql)
        {
			...
			// Add LIMIT 15 to the SQL query if it doesn't already have a LIMIT clause
			if (!sql.ToUpper().Contains("LIMIT"))
			{
				sql = sql.TrimEnd(';') + " LIMIT 15";
			}
		...
		private bool IsIdColumn(string columnName)
		{
			...
			return lowerColumnName.EndsWith("id") ||
				   lowerColumnName == "oemid" ||
				   lowerColumnName == "modelid" ||
				   lowerColumnName == "partid";
		}
